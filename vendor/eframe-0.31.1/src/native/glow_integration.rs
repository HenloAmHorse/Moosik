...
/// Moosik local patch: exposes this backend's viewport map to the shared
/// minimized-sleep policy. See `minimized_policy` and `PATCH.md`.
#[cfg(feature = "moosik_windows_linked_viewport")]
struct PolicyViewports<'a>(&'a ViewportIdMap<Viewport>);

#[cfg(feature = "moosik_windows_linked_viewport")]
impl super::minimized_policy::Viewports for PolicyViewports<'_> {
    fn count(&self) -> usize {
        self.0.len()
    }

    fn any(&self, mut f: impl FnMut(ViewportId) -> bool) -> bool {
        self.0.keys().any(|id| f(*id))
    }

    fn parent_of(&self, id: ViewportId) -> Option<ViewportId> {
        self.0.get(&id).map(|viewport| viewport.ids.parent)
    }

    fn is_deferred(&self, id: ViewportId) -> bool {
        // Deliberately not `class`: egui 0.31.1 never records
        // `ViewportClass::Immediate`, so the class is stale by the time the
        // policy runs. The callback is the same signal this backend itself
        // uses to redirect an immediate viewport's repaint to its parent.
        self.0
            .get(&id)
            .is_some_and(|viewport| viewport.viewport_ui_cb.is_some())
    }

    fn is_visible(&self, id: ViewportId) -> Option<bool> {
        self.window(id)?.is_visible()
    }

    fn is_minimized(&self, id: ViewportId) -> Option<bool> {
        self.window(id)?.is_minimized()
    }
}

#[cfg(feature = "moosik_windows_linked_viewport")]
impl PolicyViewports<'_> {
    fn window(&self, id: ViewportId) -> Option<&Window> {
        self.0.get(&id)?.window.as_deref()
    }
}
...
impl GlowWinitRunning<'_> {
    fn run_ui_and_paint(
        ...
        // Moosik local patch: keep the sleep unless a visible viewport is
        // painted by this one's pass. See `minimized_policy` and `PATCH.md`.
        let waive_minimized_sleep = {
            #[cfg(feature = "moosik_windows_linked_viewport")]
            {
                super::minimized_policy::may_waive_minimized_sleep(
                    &PolicyViewports(&glutin.viewports),
                    viewport_id,
                )
            }
            #[cfg(not(feature = "moosik_windows_linked_viewport"))]
            {
                false
            }
        };
        if !waive_minimized_sleep && window.is_minimized() == Some(true) {
...
/// Moosik local patch: the backend half of the minimized-sleep policy — that
/// this adapter reports what the policy expects. The policy itself is tested
/// in `minimized_policy`.
#[cfg(all(test, feature = "moosik_windows_linked_viewport"))]
mod policy_adapter_tests {
    use super::*;
    use crate::native::minimized_policy::{may_waive_minimized_sleep, Viewports as _};

    fn viewport(this: &str, parent: ViewportId, class: ViewportClass, deferred: bool) -> Viewport {
        let cb: Option<Arc<DeferredViewportUiCallback>> =
            deferred.then(|| Arc::new(|_: &egui::Context| {}) as Arc<DeferredViewportUiCallback>);
        Viewport {
            ids: ViewportIdPair::from_self_and_parent(ViewportId::from_hash_of(this), parent),
            class,
    ...
    fn map(viewports: Vec<Viewport>) -> ViewportIdMap<Viewport> {
        viewports.into_iter().map(|v| (v.ids.this, v)).collect()
    }

    #[test]
    fn a_stale_deferred_class_does_not_make_a_viewport_deferred() {
        // egui 0.31.1 never records `ViewportClass::Immediate`, so a viewport
        // with no callback can still be carrying an old `Deferred` class. The
        // adapter must read the callback, not the class.
        let m = map(vec![viewport(
            "spectrum",
            ViewportId::ROOT,
            ViewportClass::Deferred,
            false,
        )]);
        let ids = PolicyViewports(&m);
        assert!(!ids.is_deferred(ViewportId::from_hash_of("spectrum")));
    }

    #[test]
    fn a_present_callback_makes_a_viewport_deferred() {
        let m = map(vec![viewport(
            "child",
            ViewportId::ROOT,
            ViewportClass::Root,
        ...
        let ids = PolicyViewports(&m);
        assert!(ids.is_deferred(ViewportId::from_hash_of("child")));
    }

    #[test]
    fn the_adapter_reports_parents_and_size() {
        let m = map(vec![viewport(
            "spectrum",
            ViewportId::ROOT,
            ViewportClass::Root,
        ...
        assert_eq!(
            ids.parent_of(ViewportId::from_hash_of("spectrum")),
            Some(ViewportId::ROOT)
        );
        assert_eq!(ids.parent_of(ViewportId::from_hash_of("absent")), None);
    }

    #[test]
    fn a_viewport_without_a_window_has_no_native_state_and_keeps_the_sleep() {
        let m = map(vec![viewport(
            "spectrum",
            ViewportId::ROOT,
            ViewportClass::Root,
        ...
        let ids = PolicyViewports(&m);
        let spectrum = ViewportId::from_hash_of("spectrum");
        assert_eq!(ids.is_visible(spectrum), None);
        assert_eq!(ids.is_minimized(spectrum), None);
        // Same entry point this backend calls from its frame loop.
        assert!(!may_waive_minimized_sleep(&ids, ViewportId::ROOT));
    }
}
