...
/// Standard PCM rates probed against each device.
pub const PROBE_RATES: [u32; 8] =
...
impl BpStream {
    ...
    pub fn open(
        ...
        let (handle, format_label, dev_label) = cpal_out::open(
            device_name, sample_rate, channels, src_bits, Arc::clone(&shared), tap)?;
        #[cfg(not(windows))]
