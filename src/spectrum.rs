...
fn eq_biquad_coeffs(band: &EqBand, sr: u32) -> ([f64; 3], [f64; 2]) {
...
fn biquad_response_db(b: [f64; 3], a: [f64; 2], freq: f32, sr: u32) -> f32 {
...
fn bands_equal(a: &[EqBand], b: &[EqBand]) -> bool {
...
struct Biquad { b: [f64; 3], a: [f64; 2], z1: f64, z2: f64 }
impl Biquad {
    fn new(b: [f64; 3], a: [f64; 2]) -> Self { Self { b, a, z1: 0.0, z2: 0.0 } }
...
struct KWeightFilter { stage1: Biquad, stage2: Biquad }
impl KWeightFilter {
    fn new(sr: u32) -> Self {
        use std::f64::consts::PI;
        let sr = sr as f64;
        ...
        ctx.show_viewport_immediate(vp_id, vp_builder, |vp_ctx, _class| {
            ...
            egui::CentralPanel::default().show(vp_ctx, |ui| {
                        ...
                        ui.add(egui::Slider::new(&mut self.bar_gap, 0.0..=12.0)
                            ...
                            .on_hover_text(
                                "Pixel gap between bars.\n\
                                 At high bar counts (e.g. 1024) bars are less than 1px wide,\n\
                                 so the gap has no visible effect — this is expected."
                            );
                    });
                }
                ...
                if ui.is_rect_visible(rect) {
                    ...
                    match self.style {
                        VizStyle::Bars       => draw_bars(&painter, mags, plot_rect, self.bar_gap),
